...
def do_search(givenName):
    ...
    for num in range(1, pageNum + 1):
        ...
        # 输出 7 行回车一次
        for num, value in enumerate(find_pkg):
            print num, value
            if num % 6:
...
if __name__ == '__main__':
    ...
    if not argvs:
        argvs = ['-h']
    desc = 'Search package in Arch Linux Packages, a.k.a https://www.archlinux.org/packages.'
    parser = argparse.ArgumentParser(description=desc)
    parser.add_argument('-v', '--version', action='store_true',
                        dest='v', help='show the version number and exit')
    parser.add_argument('-f', '--fetch', action='store_true',
                        dest='f', help='download PKGBUILD and other related files')
